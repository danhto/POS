...
/*     */   }
/*     */ 
/*     */   public static int PrintReceipt(String path, String transaction, String message, String printTo)
/*     */   {
/* 105 */     initPrintValues();
/*     */ 
...
/*     */     {
                JOptionPane.showMessageDialog(JOptionPane.getRootFrame(), 
                        "<html>The printer specified cannot be found. Please check your settings. Printing has been turned off.", 
                        "Printer cannot be found.", 0);
...
/*     */   public static String displayLogDate(String startDate, String endDate, String tillAmount)
/*     */   {
/* 575 */     POSDatabase database = new POSDatabase();
/* 576 */     database.readSalesByDate();
/* 577 */     Map prices = database.readPrices();
...
/* 589 */     Map byDate = database.dateSalesFormat;
/*     */ 
/* 592 */     printString.append(String.format("%48s%s%60s%s", new Object[] { "Sales item report by date", new StringBuilder().append(newLine).append(newLine).toString(), new StringBuilder().append("Report printed on ").append(date.getTime()).toString(), new StringBuilder().append(newLine).append(newLine).toString() }));
/* 593 */     printString.append(String.format("%34s %12s to %12s%s", new Object[] { "Time span of report: ", startDate, endDate, new StringBuilder().append(newLine).append(newLine).toString() }));
/* 594 */     //printString.append(String.format("%48s $%8.2f %s", "Today's till amount subtracted from Total: ", Double.parseDouble(tillAmount), newLine+newLine));
              printString.append(String.format("%11s %15s %2s %12s %6s %6s %11s%s", new Object[] { space, "Product", space, "Quantity", space, "Amount ($)", space, newLine }));
/* 595 */     printString.append(String.format("%56s%s", new Object[] { drawLine, newLine }));
/*     */ 
/* 598 */     Iterator keyDates = byDate.keySet().iterator();
/*     */ 
...
/* 603 */       String keyDate = keyDates.next().toString().trim();
/*     */ 
/* 605 */       if (compDates(startDate, keyDate) == 0)
/*     */       {
/* 607 */         goodDates.add(keyDate);
/*     */       }
/*     */ 
...
/*     */     }
              ...
              //remove till amt from total
              BigDecimal tillAmtInBD = new BigDecimal(tillAmount); 
              totalAmount = totalAmount - tillAmtInBD.doubleValue();
/*     */ 
/* 688 */     printString.append(newLine);
/* 689 */     printString.append(String.format("%56s%s", new Object[] { drawLine, newLine }));
/* 690 */     printString.append(String.format("%16s Total sale amount for %s: %6.2f%s", new Object[] { space, startDate, Double.valueOf(totalAmount), newLine }));
/* 691 */     printString.append(String.format("%56s%s", new Object[] { drawLine, newLine }));
/*     */ 
...
/* 232 */           //g2d.drawString((String)database.getSettings().get("title"), POSPrinter.LOGO_WIDTH / 4 + 10, POSPrinter.LOGO_HEIGHT / 2);
                    g2d.drawString((String)database.getSettings().get("title"), POSPrinter.LOGO_WIDTH / 4, POSPrinter.LOGO_HEIGHT / 2);
                    g2d.setFont(new Font("Arial Black", 0, 24));
/*     */         }
/*     */         else {
